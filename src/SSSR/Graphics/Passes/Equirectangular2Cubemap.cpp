...
Equirectangular2Cubemap::Equirectangular2Cubemap(RenderDevice& device, const Input& input)
    : m_device(device)
    , m_input(input)
...
{
    if (!is || m_settings.Get<bool>("draw_Equirectangular2Cubemap_every_frame"))
    {
...
void Equirectangular2Cubemap::DrawEquirectangular2Cubemap(RenderCommandList& command_list)
{
    ...
    glm::vec4 color = { 0.0f, 0.3f, 0.0f, 1.0f };
...
void Equirectangular2Cubemap::OnModifySSSRSettings(const SSSRSettings &settings)
{
    m_settings = settings;
}
