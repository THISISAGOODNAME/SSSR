...
BRDFGen::BRDFGen(RenderDevice& device, const Input& input)
    ...
    , m_program(device)
{
    output.brdflut = m_device.CreateTexture(BindFlag::kRenderTarget | BindFlag::kShaderResource, gli::format::FORMAT_RG32_SFLOAT_PACK32, 1, m_size, m_size, 1);
...
{
    if (!is || m_settings.Get<bool>("draw_BRDFlut_every_frame"))
    {
        command_list.BeginEvent("Draw BRDF lut");
...
void BRDFGen::DrawBRDF(RenderCommandList& command_list)
{
    ...
    render_pass_desc.colors[m_program.ps.om.rtv0].texture = output.brdflut;
...
void BRDFGen::OnModifySSSRSettings(const SSSRSettings& settings)
{
    m_settings = settings;
}
