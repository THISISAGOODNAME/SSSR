...
template<typename T>
void SSSRSettings::add_combo(const std::string& label, const std::vector<std::string>& items, const std::vector<T>& items_data, T value)
{
...
HotKey& SSSRSettings::add_checkbox(const std::string& label, bool value)
{
    m_settings[label] = value;
...
void SSSRSettings::add_slider_int(const std::string& label, int32_t value, int min, int max)
{
    m_settings[label] = value;
...
void SSSRSettings::add_slider(const std::string& label, float value, float min, float max, bool linear)
{
    m_settings[label] = value;
    if (linear)
    {
    ...
    else
    {
        ...
        {
            float value = Get<float>(label);
            auto res = ImGui::SliderFloat(label.c_str(), &value, min, max, "%.3f");
            m_settings[label] = value;
            return res;
        });
    }
...
bool SSSRSettings::OnKey(int key, int action)
{
